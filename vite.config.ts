...
export default defineConfig(({ mode }) => {
  ...
  // TODO: Env toggle for dev SW: Add optional VITE_ENABLE_DEV_SW and wire devOptions.enabled in vite.config.ts to isDev && env.VITE_ENABLE_DEV_SW === 'true'. Default stays off.
  const devSW = false;
  ...
  return {
    ...
    plugins: [
      ...
      VitePWA({
        ...
        manifest: {
          id: isDev ? 'unpackd-dev' : 'unpackd',
          name: isDev ? 'Unpackd (Dev)' : 'Unpackd',
          short_name: isDev ? 'Unpackd Dev' : 'Unpackd',
          description: 'Track moving inventory via NTAG215 stickers',
          start_url: isDev ? '/#/' : '/Unpackd/#/',
          scope: isDev ? '/' : '/Unpackd/',
          theme_color: '#121212',
